...
class gtpub:
    def __init__(self):
        ...
        self.drone_name = rospy.get_param("/drone_name", "typhoon_h480")
        self.my_pose = rospy.Subscriber(
            "/gazebo/model_states", ModelStates, self.gazebo_callback
        )
        self.gtpubb = rospy.Publisher(
        ...
        try:
            if pub_class.check == 1:
                ...
                pose.pose.position.x = (
                    pub_class.pose.position.x - pub_class.pose_offset.position.x
                )
                pose.pose.position.y = (
                    pub_class.pose.position.y - pub_class.pose_offset.position.y
                )
                pose.pose.position.z = (
                    pub_class.pose.position.z - pub_class.pose_offset.position.z
                )
