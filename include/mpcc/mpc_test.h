...
class LSH_MPC{
    ...
    LSH_MPC(const ros::NodeHandle& n_private) : nh(n_private){
      ////////// ROS things
			// params
      ...
      // subscribers
      m_state_sub = nh.subscribe<mavros_msgs::State>("/mavros/state", 10, &LSH_MPC::state_cb, this);
      m_odom_sub = nh.subscribe<nav_msgs::Odometry>("/mavros/local_position/odom", 10, &LSH_MPC::odom_cb, this);
			m_target_sub = nh.subscribe<nav_msgs::Odometry>("/target_pos", 10, &LSH_MPC::target_cb, this);
			m_target_traj_sub = nh.subscribe<nav_msgs::Path>("/target_traj", 10, &LSH_MPC::target_traj_cb, this);
      ...
      m_control_timer = nh.createTimer(ros::Duration(1/ctrl_hz), &LSH_MPC::control_timer_func, this);
...
//////////////////////// callbacks
void LSH_MPC::state_cb(const mavros_msgs::State::ConstPtr& msg){
  curr_state = *msg;
  state_check = true;
}

void LSH_MPC::odom_cb(const nav_msgs::Odometry::ConstPtr& msg){
...
void LSH_MPC::target_cb(const nav_msgs::Odometry::ConstPtr& msg){
	t_time = ros::Time::now();
	...
	t_att << msg->pose.pose.orientation.w, msg->pose.pose.orientation.x, 
           msg->pose.pose.orientation.y, msg->pose.pose.orientation.z;
...
void LSH_MPC::target_traj_cb(const nav_msgs::Path::ConstPtr& msg){
	target_traj = *msg;
}

void LSH_MPC::control_timer_func(const ros::TimerEvent& event){
	if (state_check){
...
void LSH_MPC::pos_ctrl(Vector3d target_pos, Vector4d target_att){
...
void LSH_MPC::solve_mpc(){
  ...
  // setReference(reference_trajectory);
    ...
    reference_states_(1, i) = target_traj.poses[i].pose.position.y;
		...
		reference_inputs_(0, i) = 9.8066;
		reference_inputs_(1, i) = 0;
		reference_inputs_(2, i) = 0;
		reference_inputs_(3, i) = 0;
	}
  ...
  mpc_wrapper_.getInputs(predicted_inputs_);
	...
	preparation_thread_ = std::thread(&LSH_MPC::preparationThread, this);
  ...
  timing_feedback_ = 0.9 * timing_feedback_ +
	...
	if (debug)
		ROS_INFO_THROTTLE(1.0, "MPC Timing: Latency: %1.1ld ms  |  Total: %1.1ld ms",
  ...
  ctbr_cmd(INPUT::kThrust) = std::max(min_throttle_,
                                          std::min(max_throttle_, input_bounded(INPUT::kThrust)));
  ctbr_cmd(INPUT::kRateX) = std::max(-max_bodyrate_xy_,
                                          std::min(max_bodyrate_xy_, input_bounded(INPUT::kRateX)));
  ctbr_cmd(INPUT::kRateY) = std::max(-max_bodyrate_xy_,
                                          std::min(max_bodyrate_xy_, input_bounded(INPUT::kRateY)));
  ctbr_cmd(INPUT::kRateZ) = std::max(-max_bodyrate_z_,
                                          std::min(max_bodyrate_z_, input_bounded(INPUT::kRateZ)));
}

void LSH_MPC::preparationThread() {
  const clock_t start = clock();
...
//////////////////////L1////////////////////// 
void LSH_MPC::compute_L1adaptive(){
	if(!L1_start){
		L1_start = true;
		predict_vel = vel;
		predict_rot = rot;
		...
		z_error << L1_vel_error(0), L1_vel_error(1), L1_vel_error(2), 
               L1_ang_error(0), L1_ang_error(1), L1_ang_error(2);
    ...
    MatrixXd B_bar = MatrixXd::Zero(6,6);
		...
		VectorXd sigma = -B_bar.inverse()*(exp_AsTs-MatrixXd::Identity(6,6)).inverse()*As*exp_AsTs*z_error;
    ...
    mpc_wrapper_.setadaptiveacc(um_ada_cmd);
		...
		if (debug){
			...
			ROS_INFO("Adaptive command: %.2f %.2f %.2f %.2f", 
                ada_cmd(0), ada_cmd(1), ada_cmd(2), ada_cmd(3));
			ROS_INFO("Adaptive acc: %.2f %.2f %.2f", 
                um_ada_cmd(0), um_ada_cmd(1), um_ada_cmd(2));
      // ROS_INFO("Ada mag: %.2f", um_ada_cmd.norm());
		}

		//Prediction step
		predict_vel = predict_vel+(gravity + rot.col(2)*(throttle_mapping_inverse(att_msg.thrust)+m_sigma(3))+rot.col(0)*um_sigma(0)+rot.col(1)*um_sigma(1)+As.block<3,3>(0,0)*L1_vel_error)/ctrl_hz;

		predict_rot = predict_rot*exp_operator((ctbr_cmd.block<3,1>(1,0)+ada_cmd.head(3)+m_sigma.head(3)+As.block<3,3>(3,3)*L1_ang_error)/ctrl_hz);
	}
}

// Utility functions
double LSH_MPC::throttle_mapping(double thrust){
	return max(min_thrust, min(max_thrust, thrust_const * thrust + thrust_offset));
}

double LSH_MPC::throttle_mapping_inverse(double throttle){
	return (throttle - thrust_offset) / thrust_const;
}

Matrix3d LSH_MPC::hat_operator(Vector3d v){
	Matrix3d R;
	R << 0.0, -v(2), v(1), 
       v(2), 0.0, -v(0),
      -v(1), v(0), 0.0;
...
Vector3d LSH_MPC::vee_operator(Matrix3d R){
	Vector3d v;
	v << R(7), R(2), R(3);
	return v;
}

Matrix3d LSH_MPC::exp_operator(Vector3d v){
	Matrix3d v_hat;
