#ifndef INCLUDE_MPCC_BEZIER_CURVE_H_
#define INCLUDE_MPCC_BEZIER_CURVE_H_
...
void BEZIER::binCoeff() {
  static const auto factorial = [](int n) {
    int fact = 1;
  ...
  static const auto combinatorial = [](int n, int k) {
  ...
  if (traj_order > 1) C_a.resize(traj_order - 1);

  if (traj_order > 2) C_j.resize(traj_order - 2);

  for (int k = 0; k <= traj_order; k++) {
    C(k) = combinatorial(traj_order, k);

    if (k <= (traj_order - 1)) C_v(k) = combinatorial(traj_order - 1, k);
    if (k <= (traj_order - 2)) C_a(k) = combinatorial(traj_order - 2, k);
    if (k <= (traj_order - 3)) C_j(k) = combinatorial(traj_order - 3, k);
  }
}
...
#endif  // INCLUDE_MPCC_BEZIER_CURVE_H_
