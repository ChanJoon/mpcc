#ifndef WIND_DISTURBANCE_H
#define WIND_DISTURBANCE_H
...
class wind_disturbance
{
  public:
    ...
    wind_disturbance(ros::NodeHandle& n) : nh(n)
    {
      ///// params
      ...
      ///// sub pub
      states_sub = nh.subscribe<gazebo_msgs::ModelStates>("/gazebo/model_states", 3, &wind_disturbance::states_callback, this);
...
void wind_disturbance::if_wind_disturbance(const geometry_msgs::Pose &pose)
{
  ...
  {
    if ( m_wind_spec[i*9] < x && m_wind_spec[i*9+1] < y && m_wind_spec[i*9+2] < z && x < m_wind_spec[i*9+3] && y < m_wind_spec[i*9+4] && z < m_wind_spec[i*9+5])
    {
      if (m_wind_spec[i*9+6]==0.0) // +x direction
      {
        m_model_force_srv.request.wrench.force.x = 1.0 * gaussian_distribution(m_wind_spec[i*9+7], m_wind_spec[i*9+8]);
      ...
      else if (m_wind_spec[i*9+6]==1.0) //-y direction
      {
        ...
        m_model_force_srv.request.wrench.force.y = -1.0 * gaussian_distribution(m_wind_spec[i*9+7], m_wind_spec[i*9+8]);
      }
